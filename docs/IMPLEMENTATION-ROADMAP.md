...
- Convert the selected product opportunity into structured Shopee listing fields.
- Keep title, description, attributes, SKU, price, stock, and logistics aligned with platform rules.
...
- Listing must be English for MVP.
- Do not use exaggerated claims:
  - super suction
  - industrial grade
  - certified safety
  - guaranteed deep cleaning
- If Risk flags a claim, rewrite the claim before returning.
- Missing required fields must prevent ready status.

Harness assertions:

- required fields are filled
- category id exists
- price matches margin recommendation
- logistics include weight and dimensions
- risk warnings are reflected in listing notes or wording
- title and description do not contain banned exaggerated terms

Audit requirements:

- Store category attribute snapshot.
- Store validator output.
- Store listing risk checkpoint result.
- Store final listing JSON.
